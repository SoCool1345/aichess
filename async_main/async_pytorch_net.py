...
class PolicyValueNet:
    ...
    async def policy_value_fn(self, board_list):
        self.policy_value_net.eval()
        # 获取合法动作列表
        try:
        ...
        current_state_list = [torch.as_tensor(board.current_state().astype('float16')) for board in board_list]
        ...
        with autocast():  # 半精度fp16
            log_act_probs, value = await self.policy_value_net(current_state)
        log_act_probs, value = log_act_probs.cpu(), value.cpu()
        act_prob_list = []
        for i in range(len(log_act_probs)):
            act_probs = np.exp(log_act_probs[i].flatten()) if CONFIG['use_frame'] == 'paddle' else np.exp(
                log_act_probs[i].detach().flatten())
            # 只取出合法动作
            act_probs = zip(legal_positions[i], act_probs[legal_positions[i]])
            act_prob_list.append(act_probs)
        # 返回动作概率，以及状态价值
        return act_prob_list, value.detach()
